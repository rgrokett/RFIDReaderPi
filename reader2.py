...
def led(value):
	...
	if (DEBUG): print "LED state is : HIGH"
    else:
        GPIO.output(LED, GPIO.LOW)
	if (DEBUG): print "LED state is : LOW"
...
def buzzer(value):
	...
	if (DEBUG): print "BUZZER state is : HIGH"
    else:
        GPIO.output(BUZZER, GPIO.LOW)
	if (DEBUG): print "BUZZER state is : LOW"

# TURN ON/OFF GPIO RELAY
def relay(value):
    if (DEBUG): print "relay()"
    if (value):
        GPIO.output(RELAY, GPIO.HIGH)
	if (DEBUG): print "RELAY state is : HIGH"
    else:
        GPIO.output(RELAY, GPIO.LOW)
	if (DEBUG): print "RELAY state is : LOW"

# TURN ON/OFF I2C RELAY
def i2c_relay(value):
    if (DEBUG): print "i2c_relay()"
    if not I2C:
	return
	...
	if (DEBUG): print "Relay-1 state is : HIGH"
    else:
	# PCA9536_R11 address, i2c_addr(65)
	# Select output port register, 0x01(01)
	bus.write_byte_data(i2c_addr, 0x01, 0x00)
	if (DEBUG): print "Relay-1 state is : LOW"

# UNLOCK DOOR
def unlock_door():
    if (DEBUG): print "unlock_door()"
    led(ON)
    buzzer(ON)
    relay(ON)
    i2c_relay(ON) 
...
if __name__ == '__main__':
    ...
    if (GPIO.RPI_REVISION == 3):
        # For Pi3 use
	if (DEBUG): print "This is a Pi 3"
        ser = serial.Serial('/dev/ttyS0', 9600, timeout=1)
    else:
        # For Pi2, B+, Zero use
	if (DEBUG): print "This is a Pi B+, 2, or Zero"
	...
	with open(CARDFILE) as lines:
		...
		if '=' in line:
			...
			pass
	if (DEBUG): print "Loaded "+str(cnt)+" cards"
    except:
	if (DEBUG): print "Could not read ", CARDFILE
    ...
    while True:
        ...
        if len(buffer) == 0:
	    if (DEBUG): print "Please wave a tag"
	    
        else:
	    card_id = buffer[1:11]	# Strip header/trailer
	    if (DEBUG): print "Card ID : ", card_id
	    if (card_id in dict.keys(CARDS)):
	        if (DEBUG): print "FOUND CARD ID: ", card_id
		write_log(card_id,CARDS[card_id])
	        unlock_door()
	        time.sleep(5)
	        lock_door()
	    else:
	        if (DEBUG): print "CARD NOT IN LIST!"
